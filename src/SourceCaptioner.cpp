...
using TextOutputTup = std::tuple<string, string>;
...
last_caption_cleared(true) {
    ...
    QObject::connect(this,&SourceCaptioner::audio_capture_status_change,this,&SourceCaptioner::process_audio_capture_status_change);
...
void SourceCaptioner::stop_caption_stream(bool send_signal) {
    ...
    if (send_signal) {
        emit source_capture_status_change(std::make_shared<SourceCaptionerStatus>(
...
bool SourceCaptioner::_start_caption_stream(bool restart_stream) {
    debug_log("_start_caption_stream");
    ...
    bool caption_settings_equal;
    {
        ...
        try {
            ...
            audio_chunk_data_cb audio_cb = std::bind(&SourceCaptioner::on_audio_data_callback,this, std::placeholders::_1, std::placeholders::_2,std::placeholders::_3);
            ...
            if (use_output_audio) {
                ...
                output_audio_capture_session = std::make_unique<OutputAudioCaptureSession>(
                    track_index,audio_cb,audio_status_cb,resample_to,audio_capture_id);
            }else {
                source_audio_capture_session = std::make_unique<SourceAudioCaptureSession>(
                    caption_source,mute_source,audio_cb,audio_status_cb,resample_to,MUTED_SOURCE_REPLACE_WITH_ZERO,false,audio_capture_id);
            }
...
void SourceCaptioner::on_audio_capture_status_change_callback(const int id, const audio_source_capture_status status) {
    emit audio_capture_status_change(id,status);
}

void SourceCaptioner::process_audio_capture_status_change(const int id, const int new_status) {
    settings_change_mutex.lock();
    bool is_old_audio_session = id != audio_capture_id;
    SourceCaptionerSettings cur_Settings = settings;
...
void SourceCaptioner::clear_output_timer_cb() {
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(settings_change_mutex);
        if (!this->settings.format_settings.caption_timeout_enabled || this->last_caption_cleared)
            return;

        double secs_since_last_caption = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now()-this->last_caption_at).count();
...
void SourceCaptioner::process_caption_result(const CaptionResult caption_result, bool interrupted) {
    ...
    vector<TextOutputTup> text_source_sets;
    {
        ...
        native_output_result = caption_result_handler->prepare_caption_output(caption_result,true,
            ...
            settings.format_settings.capitalization_type,
            interrupted,
            results_history);
        if (!native_output_result)
            return;
        const SceneCollectionSettings &scene_col_settings = this->settings.get_scene_collection_settings(selected_scene_collection_name);
        for (const auto &text_output :scene_col_settings.text_outputs) {
            if (!text_output.isValidEnabled())
                continue;
            auto text_output_result = caption_result_handler->prepare_caption_output(caption_result,true,
                true,
                text_output.insert_punctuation,
                text_output.line_length,
                text_output.line_count,
                text_output.capitalization,
                interrupted,
                results_history);
            text_source_sets.emplace_back(text_output.text_source_name,text_output_result->output_line);
        }
...
void SourceCaptioner::output_caption_writers(const CaptionOutput &output, bool to_stream, bool to_recoding, bool to_transcript_streaming, bool to_transcript_recording, bool to_transcript_virtualcam, bool is_clearance) {
