...
struct CaptionSourceSettings {
    ...
    bool operator==(const CaptionSourceSettings &other) const {
        return caption_source_name == other.caption_source_name &&
            mute_when == other.mute_when &&
                mute_source_name == other.mute_source_name;
    }
    ...
    [[nodiscard]] string active_mute_source_name() const {
...
struct CaptionOutput {
    ...
    output_result(output_result),is_clearance(is_clearance) {}
...
struct OutputWriter {
    ...
    void clear() {
        std::lock_guard<std::recursive_mutex> lock(control_change_mutex);
        if (control) {
            control->stop_soon();
            control = nullptr;
        }
    ...
    void set_control(std::shared_ptr<CaptionOutputControl<T>> &set_control) {
        std::lock_guard<std::recursive_mutex> lock(control_change_mutex);
        clear();
...
struct SourceCaptionerStatus {
    ...
    SourceCaptionerStatus(SourceCaptionerStatusEvent event_type, bool settings_changed, bool stream_settings_changed,
        const SourceCaptionerSettings &settings,string scene_collection_name,audio_source_capture_status audio_capture_status,bool active) :
        event_type(event_type),settings_changed(settings_changed),stream_settings_changed(stream_settings_changed),
        settings(settings),scene_collection_name(scene_collection_name),audio_capture_status(audio_capture_status),active(active)
    {}
};
...
class SourceCaptioner : public QObject {
    ...
    std::vector<std::shared_ptr<OutputCaptionResult>> results_history;
...
private slots:
    ...
    void audio_capture_status_change(const int id,const int new_status);

    void source_capture_status_change(shared_ptr<SourceCaptionerStatus> status);
public:
