...
static bool set_WordReplacements(obs_data_t *save_data, const std::vector<WordReplacement> &word_replacements);
...
static DefaultReplacer makeDefaultReplacer(const vector<WordReplacement>& userReplacements) {
    return DefaultReplacer({"niger", "nigger", "nigga", "niggas", "\\bfag\\b", "faggot", "chink"},userReplacements);
}
...
static CaptionPluginSettings default_CaptionPluginSettings() {
    SourceCaptionerSettings default_source_captioner_settings = default_SourceCaptionerSettings();
    return CaptionPluginSettings(
            false,
            default_source_captioner_settings
    );
}
...
static void enforce_CaptionPluginSettings_values(CaptionPluginSettings &settings) {
    ...
    if (source_settings.format_settings.capitalization_type < 0 || source_settings.format_settings.capitalization_type > 2) {
        source_settings.format_settings.capitalization_type = static_cast<CapitalizationType>(0);
    }

    if (source_settings.transcript_settings.srt_capitalization < 0 || source_settings.transcript_settings.srt_capitalization > 2) {
        source_settings.transcript_settings.srt_capitalization = static_cast<CapitalizationType>(0);
    }
...
static void enforce_TextOutputSettings_values(TextOutputSettings &settings) {
    if (settings.capitalization < 0 || settings.capitalization > 2)
        settings.capitalization = static_cast<CapitalizationType>(0);
}
...
static SceneCollectionSettings get_SceneCollectionSettings_from_data(obs_data_t *load_data) {
    ...
    caption_source_settings.mute_when = string_to_mute_settings(mute_when_str,CAPTION_SOURCE_MUTE_TYPE_FROM_OWN_SOURCE);
...
static CaptionPluginSettings get_CaptionPluginSettings_from_data(obs_data_t *load_data) {
    ...
    obs_data_set_default_int(load_data, "caption_capitalization", source_settings.format_settings.capitalization_type);
    ...
    source_settings.format_settings.capitalization_type = (CapitalizationType) obs_data_get_int(load_data, "caption_capitalization");
...
//                printf("adding fallback %s\n", word.c_str());
    ...
    source_settings.format_settings.default_replacer = makeDefaultReplacer(word_reps);
...
static void set_CaptionPluginSettings_on_data(obs_data_t *save_data, const CaptionPluginSettings &settings) {
    ...
    obs_data_set_int(save_data, "caption_capitalization", source_settings.format_settings.capitalization_type);
//    obs_data_set_bool(save_data, "caption_insert_newlines", settings.format_settings.caption_insert_newlines);
    ...
    set_WordReplacements(save_data, source_settings.format_settings.default_replacer.user_replacements());
...
bool get_WordReplacements(obs_data_t *load_data, std::vector<WordReplacement> &word_placements) {
    ...
    for (size_t index = 0;index < obs_data_array_count(words_array);++index) {
        ...
        if (!type.empty() && !from.empty()) {
            word_placements.emplace_back(type, from, to);
        }
...
bool set_WordReplacements(obs_data_t *save_data, const std::vector<WordReplacement> &word_replacements) {
    obs_data_array_t *scene_collection_sources_array = obs_data_array_create();

    for (auto &word:word_replacements) {
...
static CaptionPluginSettings load_CaptionPluginSettings_from_config() {
    if (obs_data_t *loaded_data = load_plugin_data_from_config()) {
