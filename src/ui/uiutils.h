...
static vector<ObsSourceTup> get_obs_sources () {
    ...
    auto cb = [](void* param, obs_source_t *source) {
        ...
        sources->emplace_back(string(name),string(source_type),caps);
        return true;
    };
...
static vector<string> get_audio_sources () {
    ...
    for (auto &a_source:sources) {
        if (std::get<2>(a_source) && OBS_SOURCE_AUDIO) {
            audio_sources.emplace_back(std::get<0>(a_source));
        }
    }
...
static vector<string> get_text_sources () {
    ...
    for (auto &a_source:sources) {
        ...
        if (source_type == "text_gdiplus" || source_type == "text_gdiplus_v2"
            ...
            text_sources.emplace_back(std::get<0>(a_source));
    }
    return text_sources;
}

static void setup_combobox_texts(QComboBox &combobox, const vector<string> &items) {
...
static bool captioning_status_string(
    ...
    if (!enabled)
        ...
        if (status.event_type == SOURCE_CAPTIONER_STATUS_EVENT_STOPPED
            ...
            const string source_name = corrected_streaming_audio_output_capture_source_name(status.settings.scene_collection_settings.caption_source_settings.caption_source_name);
