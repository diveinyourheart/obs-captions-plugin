...
static void caption_output_writer_loop(shared_ptr<CaptionOutputControl<int>> control, bool to_stream) {
    ...
    while (!control->stop) {
        ...
        if (!caption_output.is_clearance && caption_output.output_result->output_line.empty()) {
            debug_log("ignoring empty non clearance, %s", to_what.c_str());
            continue;
        }
        ...
        if (active_delay_sec) {
            ...
            if (wait_left > wanted_delay) {
                info_log("capping delay, negative duration?, got %f, max %f",
                ...
                waited_left_secs = chrono::duration_cast<chrono::duration<double>>(wait_left).count();
